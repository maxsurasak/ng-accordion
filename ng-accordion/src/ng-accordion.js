        ...
        link: function (scope, elem, attrs) {
          if (attrs.handle) {
            ...
            function onCollapse(index) {
              ...
              if (scope.timing)
                $timeout(function () {
                  scope.$root[attrs.handle].$emit(attrs.handle + ':collapse:animation', index);
                }, scope.timing);
            }
            ...
            function onExpand(index) {
              ...
              if (scope.timing)
                $timeout(function () {
                  scope.$root[attrs.handle].$emit(attrs.handle + ':expand:animation', index);
                }, scope.timing);
            }
      ...
      return {
        ...
        link: function (scope, elem, attrs) {
          ...
          scope.$watch('toggle', function (n, o) {
            ...
            var css = n ? {
              'max-height': elem[0].scrollHeight + 'px',
              'transition': 'max-height ' + scope.timing + 'ms'
            } : {
              'max-height': 0,
              'transition': 'max-height ' + scope.timing + 'ms'
            }
